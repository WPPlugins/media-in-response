...
var FancyUpload2 = new Class({
	...
	initialize: function(status, image, imagename, message, options) {
		this.status = $(status);
		this.image = $(image);
	...
	onProgress: function(file, current, overall) {
		//this.overallProgress.start(overall.bytesLoaded, overall.bytesTotal); //XXX
    ...
    this.log('Stored as:'+filename);
    
    this.log('Images field:'+this.imagename.id);
    
    this.log('File value:'+this.imagename.value);
    this.imagename.value = filename;
    this.log('File value:'+this.imagename.value);
    this.imagename.setProperty('value',filename);
    this.log('File value:'+this.imagename.value);
    
    
    var src = json.get('src');
    
    this.log('Preview value:'+src);
    this.log('Preview:'+this.image.src);
    this.image.setProperty('src',src);
    this.log('Preview:'+this.image.src);

    //this.message.class = 'visible';
    //this.message.toggleClass('visible');
